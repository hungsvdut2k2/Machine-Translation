...
class NMTDataset(Dataset):
    def __init__(self, cfg, data_type="train"):
        ...
        (
            self.tgt_input_ids,
            self.tgt_attention_mask,
            self.labels,
        ) = self.texts_to_sequences(self.tgt_texts, is_src=False)
    ...
    def texts_to_sequences(self, texts, is_src=True):
        if is_src:
            ...
            return (src_inputs.input_ids, src_inputs.attention_mask)
            ...
            labels = [
                [
                    -100
                    if token_id == self.cfg.tgt_tokenizer.pad_token_id
                    else token_id
            ...
            return (tgt_inputs.input_ids, tgt_inputs.attention_mask, labels)

import os
import numpy as np
from data.nmt_dataset import NMTDataset
from datasets import load_metric
from utils import load_tokenizer, postprocess_text
...
class Manager:
    ...
    def get_tokenizer(self):
        ...
        self.cfg.src_tokenizer.save_pretrained(
            os.path.join(
                self.cfg.ckpt_dir, f"{self.cfg.src_lang}_tokenizer_{cfg.src_model_name}"
            )
        )

        self.cfg.tgt_tokenizer.save_pretrained(
            os.path.join(
                self.cfg.ckpt_dir, f"{self.cfg.tgt_lang}_tokenizer_{cfg.tgt_model_name}"
            )
        )
    ...
    def get_model(self):
        if self.cfg.load_model_from_path:
            ...
            self.model = EncoderDecoderModel.from_encoder_decoder_pretrained(
                self.cfg.src_model_name, self.cfg.tgt_model_name
            )
    ...
    def train(self):
        ...
        data_collator = DataCollatorForSeq2Seq(self.cfg.tgt_tokenizer, model=self.model)
    ...
    def compute_metrics(self, eval_preds):
        ...
        if isinstance(preds, tuple):
            preds = preds[0]
        decoded_preds = self.cfg.tgt_tokenizer.batch_decode(
            preds, skip_special_tokens=True
        )

        labels = np.where(labels != -100, labels, self.cfg.tgt_tokenizer.pad_token_id)
        decoded_labels = self.cfg.tgt_tokenizer.batch_decode(
        ...
        prediction_lens = [
            np.count_nonzero(pred != self.cfg.tgt_tokenizer.pad_token_id)
            for pred in preds
        ]
